...
@interface _RQUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _RQUIApplicationNetworkIndicatorInfo
@end


@implementation UIApplication (RQKit)
...
- (BOOL)isPirated {
    ...
    if (![self _RQ_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _RQ_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_RQ_fileExistInMainBundle:(NSString *)name {
...
RQSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _RQUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _RQUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_RQUIApplicationNetworkIndicatorInfo new];
