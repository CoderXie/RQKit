...
/**
 RQKVStorageItem is used by `RQKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface RQKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `RQKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache),
   use RQKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use RQKVStorageTypeFile to get better performance.
 * You can use RQKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, RQKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    RQKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    RQKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    RQKVStorageTypeMixed = 2,
};



/**
 RQKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for RQKVStorage is `initWithPath:type:`.
 ...
 */
@interface RQKVStorage : NSObject
...
@property (nonatomic, readonly) RQKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(RQKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is RQKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is RQKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is RQKVStorageTypeMixed, then the item.value will be saved to file
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(RQKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 RQKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is RQKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is RQKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is RQKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable RQKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable RQKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `RQKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<RQKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `RQKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<RQKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
