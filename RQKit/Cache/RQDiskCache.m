...
static int64_t _RQDiskSpaceFree() {
    NSError *error = nil;
...
static void _RQDiskCacheInitGlobal() {
...
static RQDiskCache *_RQDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _RQDiskCacheInitGlobal();
...
static void _RQDiskCacheSetGlobal(RQDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _RQDiskSpaceFree();
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"RQDiskCache init error" reason:@"RQDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    RQDiskCache *globalCache = _RQDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    RQKVStorageType type;
    if (threshold == 0) {
        type = RQKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = RQKVStorageTypeSQLite;
    } else {
        type = RQKVStorageTypeMixed;
    }
    
    RQKVStorage *kv = [[RQKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
    
    _kv = kv;
    _path = path;
    _lock = dispatch_semaphore_create(1);
    _queue = dispatch_queue_create("com.rqkit.cache.disk", DISPATCH_QUEUE_CONCURRENT);
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    RQKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [RQDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [RQDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != RQKVStorageTypeSQLite) {
