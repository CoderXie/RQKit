...
@protocol BASIC_RQError

+ (NSError *)invalidArgumentErrorWithName:(NSString *)name value:(id)value message:(NSString *)message;

@end

@implementation RQBasicUtility

+ (NSString *)JSONStringForObject:(id)object
                            error:(NSError *__autoreleasing *)errorRef
             invalidObjectHandler:(RQInvalidObjectHandler)invalidObjectHandler
{
  if (invalidObjectHandler || ![NSJSONSerialization isValidJSONObject:object]) {
    ...
    if (![NSJSONSerialization isValidJSONObject:object]) {
      if (errorRef != NULL) {
        Class RQErrorClass = NSClassFromString(@"RQError");
        if ([RQErrorClass respondsToSelector:@selector(invalidArgumentErrorWithName:value:message:)]) {
          *errorRef = [RQErrorClass invalidArgumentErrorWithName:@"object"
                                                              value:object
    ...
    [(NSDictionary<id, id> *)object enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *dictionaryStop) {
      [self dictionary:dictionary
             setObject:[self _convertObjectToJSONObject:obj invalidObjectHandler:invalidObjectHandler stop:&stop]
                forKey:[RQTypeUtility stringValue:key]];
      if (stop) {
        *dictionaryStop = YES;
      }
    }];
...
{
  NSString *file = [[self class] persistenceFilePath:RQ_BASICUTILITY_ANONYMOUSIDFILENAME];
  NSString *content = [[NSString alloc] initWithContentsOfFile:file
                                                      encoding:NSASCIIStringEncoding
                                                         error:nil];
  NSDictionary<NSString *, id> *results = [RQBasicUtility objectForJSONString:content error:NULL];
  return results[RQ_BASICUTILITY_ANONYMOUSID_KEY];
}
...
{
  NSDictionary<NSString *, NSString *> *data = @{ RQ_BASICUTILITY_ANONYMOUSID_KEY : anonymousID };
  NSString *content = [self JSONStringForObject:data error:NULL invalidObjectHandler:NULL];

  [content writeToFile:[[self class] persistenceFilePath:RQ_BASICUTILITY_ANONYMOUSIDFILENAME]
            atomically:YES
              encoding:NSASCIIStringEncoding
                 error:nil];
}
